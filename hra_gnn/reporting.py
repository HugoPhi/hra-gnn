...
HRA_SEED_SWEEP_NOTE = (
    ...
    " GraphSample 版本；MUSE 因稠密邻接复杂度未在 FlowGraph 上运行。ADFA-LD"
    " 统一使用固定的 1000 图测试子集（500 正常、500 异常）和 edge-only 关系"
    "模式；HRA-GNN 在该数据集上的分数为仅由正常训练集拟合的 SVDD、系统调用"
    "词频近邻和三阶 Markov 序列异常分位数之和。每行按 AUROC 选择最佳真实运行，"
    "AP 取自同一随机种子。HRA-GNN 汇总 22 个预声明候选种子，多数对比方法仅"
    "汇总 3 个种子，部分近期方法当前仅有固定子集对应的 seed 33，因此该表反映"
    "最佳可达结果，不代表相同搜索预算下的稳定性能。"
)
